...
import com.example.firebase_project.databinding.ActivityFaceBinding;
...
public class faceActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String faceTitle = intent.getStringExtra("faceTitle");
        binding.tvTitleface.setText(faceTitle);

        getListOfFace();
    }

    private void getListOfFace() {
        FirebaseFirestore firebaseFirestoreDB = FirebaseFirestore.getInstance();

        // قراءة البيانات سواء لييست أو أيتم واحد
        firebaseFirestoreDB.collection("faceProduct")
                        ...
                        if (task.isSuccessful()) {
                            ...
                            viewProductAdapter adapter = new viewProductAdapter(product,getBaseContext(),  new Listener() {
                                ...
                                public void onFavourite(int position, productModel productModel) {
                                    ...
                                    if (user == null){
                                        Toast.makeText(faceActivity.this, "you are not logged in", Toast.LENGTH_SHORT).show();
                                    }
                                    else{
                                        productModel productModel1 = product.get(position);
                                                ...
                                                .addOnCompleteListener(new OnCompleteListener<DocumentReference>() {
                                                    ...
                                                    public void onComplete(@NonNull Task<DocumentReference> task) {
                                                        if (task.isSuccessful()){
                                                            ...
                                                            Toast.makeText(faceActivity.this, "added to list Successfully", Toast.LENGTH_SHORT).show();
                                                        }else {
                                                            String error = task.getException().getMessage();
                                                            Toast.makeText(faceActivity.this, error, Toast.LENGTH_SHORT).show();
                                                        }
                                                    }
                                                });
                                    }
                            ...
                            binding.RVFaceView.setAdapter(adapter);
                            binding.RVFaceView.setHasFixedSize(true);
                            binding.RVFaceView.setLayoutManager(new GridLayoutManager(getBaseContext(),2));
